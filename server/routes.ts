...
router.get('/stats', (req, res) => {
  ...
  const activeVehicles = (db.prepare('SELECT COUNT(*) as c FROM vehicles WHERE status = ?').get('active') as any).c;
  const sectorsMonitored = (db.prepare('SELECT COUNT(*) as c FROM sectors').get() as any).c;
  const highConfidence = (db.prepare('SELECT COUNT(*) as c FROM reports WHERE confidence > 80').get() as any).c;
  const pendingReview = (db.prepare('SELECT COUNT(*) as c FROM reports WHERE status = ?').get('pending') as any).c;
  const resolved = (db.prepare('SELECT COUNT(*) as c FROM reports WHERE status = ?').get('resolved') as any).c;

  res.json({
    ok: true,
    data: { totalReports, activeVehicles, sectorsMonitored, highConfidence, pendingReview, resolved }
  });
});

// /api/sectors
router.get('/sectors', (req, res) => {
  const sectors = db.prepare('SELECT * FROM sectors').all().map((row: any) => ({
    ...row,
    bounds: { startLat: row.startLat, startLng: row.startLng, endLat: row.endLat, endLng: row.endLng }
  }));
...
router.get('/reports', (req, res) => {
  ...
  const reports = db.prepare(query).all(...params).map((r: any) => {
    const vehicles = db.prepare('SELECT vehicleRef FROM report_vehicles WHERE reportId = ?').all(r.id).map((v: any) => v.vehicleRef);
    return { ...r, rawDataShared: !!r.rawDataShared, reportingVehicles: vehicles };
  });
...
router.post('/reports', (req, res) => {
  ...
  if (matchedReport) {
    ...
    const existingVehicle = db.prepare('SELECT * FROM report_vehicles WHERE reportId = ? AND vehicleRef = ?').get(matchedReport.id, vehicleRef);
    
    if (!existingVehicle) {
      db.prepare('INSERT INTO report_vehicles (reportId, vehicleRef) VALUES (?, ?)').run(matchedReport.id, vehicleRef);
      db.prepare('UPDATE reports SET independentReports = independentReports + 1 WHERE id = ?').run(matchedReport.id);
      
      const updated = db.prepare('SELECT * FROM reports WHERE id = ?').get(matchedReport.id) as any;
      
      // Auto-escalation Logic
      if (updated.independentReports >= 3 && updated.status === 'pending') {
        db.prepare('UPDATE reports SET status = ? WHERE id = ?').run('under_review', matchedReport.id);
    ...
    return res.json({ ok: true, data: { id: matchedReport.id, corroborated: true } });
  }
  ...
  const insertReportVehicle = db.prepare(`INSERT INTO report_vehicles (reportId, vehicleRef) VALUES (?, ?)`);
  if (data.reportingVehicles && Array.isArray(data.reportingVehicles)) {
    data.reportingVehicles.forEach((v: string) => insertReportVehicle.run(id, v));
  } else {
