...
import { MapContainer, TileLayer, Polyline, Tooltip as LeafletTooltip, useMap } from 'react-leaflet';
...
const MapController: React.FC<{ activeSectorId: string | null }> = ({ activeSectorId }) => {
  const map = useMap();
...
export const SectorMap: React.FC = () => {
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      case 'defect': return '#dc2626'; // --colour-danger
      case 'caution': return '#d97706'; // --colour-warning
      case 'normal':
      default: return '#16a34a'; // --colour-ok
    }
  };
  ...
  return (
    <div className={styles.container}>
      <div className={styles.header}>
        ...
        {activeSectorFilter && (
          <button 
            style={{ fontSize: 'var(--type-caption)', padding: 'var(--space-1) var(--space-2)', cursor: 'pointer' }}
            ...
            return (
              ...
              >
                <LeafletTooltip sticky className={styles.tooltipContainer}>
                  <div className={styles.tooltipTitle}>{sector.displayName}</div>
                  <div className={styles.tooltipRow}>
                    <span>Status:</span>
                    <span className={styles.tooltipValue} style={{ color: getStatusColor(sector.status) }}>
                      {sector.status.toUpperCase()}
                    </span>
                  </div>
                  <div className={styles.tooltipRow}>
                    <span>Reports:</span>
                    <span className={styles.tooltipValue}>{sector.reportCount}</span>
                  </div>
                </LeafletTooltip>
              </Polyline>
            );
      ...
      <div className={styles.legend}>
        ...
        <div className={styles.legendItem}>
          <div className={styles.legendColor} style={{ backgroundColor: '#d97706' }}></div>
          <span>Caution</span>
        </div>
        <div className={styles.legendItem}>
