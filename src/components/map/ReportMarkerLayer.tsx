...
export const ReportMarkerLayer: React.FC<ReportMarkerLayerProps> = ({ reports }) => {
  return (
    <>
      {reports.map((report) => {
        ...
        return (
          ...
          >
            <Popup>
              <div className={styles.tooltipContainer} style={{ minWidth: '200px' }}>
                ...
                <div className={styles.popupContent}>
                  <div className={styles.tooltipRow}>
                    <span>Date:</span>
                    <span className={styles.tooltipValue}>{report.reportDate}</span>
                  </div>
                  <div className={styles.tooltipRow}>
                    <span>Confidence:</span>
                    <span className={styles.tooltipValue}>{report.confidence}%</span>
                  </div>
                  <div className={styles.tooltipRow}>
                    <span>Status:</span>
                    <span className={styles.tooltipValue}>{report.status.replace('_', ' ')}</span>
                  </div>
                  <div style={{ marginTop: '8px', paddingTop: '8px', borderTop: '1px solid #e2e8f0' }}>
                    <div style={{ fontSize: '10px', color: '#64748b', marginBottom: '4px' }}>
                      {vehicleCount === 1 ? '⚠ Awaiting Corroboration' : `Corroborated by ${vehicleCount} vehicles`}
                    </div>
                    {vehicleCount > 1 && (
                      <div style={{ display: 'flex', flexWrap: 'wrap', gap: '2px' }}>
                        {report.reportingVehicles?.map(v => (
                          <span key={v} style={{ fontSize: '9px', padding: '1px 4px', background: '#f1f5f9', border: '1px solid #cbd5e1', borderRadius: '4px', fontFamily: 'monospace' }}>
                            {v.substring(0, 5)}
                          </span>
                        ))}
                      </div>
                    ...
                    {vehicleCount >= 4 && (
                      <div style={{ marginTop: '4px', fontSize: '10px', color: '#16a34a', fontWeight: 'bold' }}>
                        ✓ Verified
                      </div>
                    )}
                  </div>
