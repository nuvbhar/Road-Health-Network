...
export const ReportRow: React.FC<ReportRowProps> = React.memo(({ report, onClick }) => {
  ...
  const typeInfo = REPORT_TYPES[report.type] || REPORT_TYPES['POTENTIAL_POTHOLE'];
  ...
  return (
    <div className={`${styles.row} ${onClick ? styles.clickable : ''}`} onClick={() => onClick?.(report)}>
      <div className={styles.cellMain}>
        ...
        <div className={styles.reference}>{report.latitude.toFixed(5)}, {report.longitude.toFixed(5)}</div>
      </div>
      ...
      <div className={styles.cell}>
        <div className={styles.confidenceValue} style={{ color: 'var(--text-primary)' }}>
          {report.confidence}%
        </div>
      </div>
      ...
      <div className={styles.cell}>
        <div className={styles.cellValue} style={{ fontWeight: 600 }}>
          {report.weight !== undefined ? report.weight.toFixed(2) + 'g' : 'N/A'}
        </div>
      </div>
      ...
      <div className={styles.cell}>
        <div className={styles.corroborationStack}>
          ...
          {report.independentReports > 2 && (
            <span className={styles.vehiclePillMore} style={{ padding: '0 6px' }}>
      ...
      <div className={styles.cell} style={{ alignItems: 'flex-end', paddingRight: 'var(--space-4)' }}>
