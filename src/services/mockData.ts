...
export const mockSectors: Sector[] = Array.from({ length: 12 }, (_, i) => {
  ...
  return {
    ...
    confidence:
      statuses[i] !== "normal" ? 50 + Math.floor(Math.random() * 40) : 0,
...
export const mockReports: Report[] = Array.from({ length: 30 }, (_, i) => {
  ...
  return {
    ...
    latitude:
      sector.bounds.startLat +
      (sector.bounds.endLat - sector.bounds.startLat) * Math.random(),
    longitude:
      sector.bounds.startLng +
      (sector.bounds.endLng - sector.bounds.startLng) * Math.random(),
    reportingVehicles: Array.from({ length: independentReportsCount }, (_, j) => ({
      id: generateMockUUID(),
      offsetMeters: parseFloat((Math.random() * 20).toFixed(1)),
      timestamp: new Date(Date.now() - (i * 15 + j * 5) * 60000).toISOString()
    })),
