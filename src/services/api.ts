import { AppStats, Sector, Report, Vehicle, TrendDataPoint } from '../store/types';
...
export async function fetchSectors(): Promise<Sector[]> {
  ...
  const json = await res.json();
  if (!json.ok) throw new Error(json.error?.message || 'Failed to fetch sectors');
  return json.data;
}

export async function fetchReports(filters?: Record<string, string>): Promise<Report[]> {
  const qs = new URLSearchParams(filters);
  const res = await fetch(`${API_BASE}/reports?${qs}`);
  const json = await res.json();
  if (!json.ok) throw new Error(json.error?.message || 'Failed to fetch reports');
  return json.data;
}
...
export async function fetchVehicles(): Promise<Vehicle[]> {
  ...
  const json = await res.json();
  if (!json.ok) throw new Error(json.error?.message || 'Failed to fetch vehicles');
  return json.data;
}
...
export async function createReport(report: Partial<Report>): Promise<{ id: string }> {
